...
| `CLUSTER_ENDPOINT` | deploy | — | Aurora writer endpoint (PR-B2 부터 RDS Proxy endpoint). |
| `BEDROCK_KB_ID` | deploy | — | Bedrock Knowledge Base id (Aurora pgvector backend). Read at synth time by Api stack instead of CFN cross-stack import to avoid blocking KB replacement. After Ingestion deploy, copy `Ym-Dev-Ingestion.LegalKbId` output into `.env`, then redeploy Api. |
