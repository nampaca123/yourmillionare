...
PATCH  /tenants/{id}/fx/accounts/{aid}/balance    — manual 잔액 갱신
DELETE /tenants/{id}/fx/accounts/{aid}            — manual soft delete
POST   /tenants/{id}/fx/strategy  (SSE Function URL)
                                                  — exposure_summary / convert_now_check / monthly_outlook
```
...
| `CODEF_CREDENTIAL_SECRET_ARN` | deploy | — | Secrets Manager ARN for CODEF client credentials |
...
- **CODEF 외화 거래 분기**: PR6.A에서 column 준비 완료. resAccountDeposit/resForeignCurrency 응답 파싱은 다음 슬라이스.
