...
## 2.3 MVP 범위 — OUT (Phase 3 이후)

명시적으로 **빼는** 것들 (페르소나에 불필요 + 비용 부담):

- 영수증 사진 OCR (카드/통장 API로 95%는 커버됨, OCR은 Textract 비용 발생)
- 인사·급여·4대보험 (직원 채용 후 필요)
- 재고·생산 관리 (앱 개발사라 재고 없음)
- 정식 세무 신고 직접 제출 (제휴 세무사 연계로 우회)
- 투자자용 IR 리포트 (성장 후 필요)
- 다국어, 다국가 세제
...
| Aurora Serverless v2 | 0.5 ACU avg, 100사용자 분담 | $0.44 |
| S3 + 전송 | 100MB | $0.005 |
