...
```bash
cd /Users/nampaca123/Desktop/CodeWork/yourmillionare/infrastructure
AWS_PROFILE=yn-dev npm run synth 2>&1 | grep -E "AwsSolutions-WAF|WafConstruct" | head -20
```
...
```bash
cd /Users/nampaca123/Desktop/CodeWork/yourmillionare/infrastructure
AWS_PROFILE=yn-dev npm run synth
```
...
```bash
cd /Users/nampaca123/Desktop/CodeWork/yourmillionare/infrastructure
AWS_PROFILE=yn-dev npx cdk diff 'Ym-Dev-*' 2>&1 | tee /tmp/yn-dev-prA-diff.txt
```
...
```bash
AWS_PROFILE=yn-dev npx cdk diff 'Ym-Prod-*' 2>&1 | tail -100
```
...
```bash
cp /tmp/yn-dev-prA-diff.txt /tmp/yn-prA-diff-$(date +%Y%m%d).txt
```
...
```bash
cd /Users/nampaca123/Desktop/CodeWork/yourmillionare/infrastructure
AWS_PROFILE=yn-dev npx cdk deploy 'Ym-Dev-*' --require-approval never 2>&1 | tee /tmp/yn-dev-prA-deploy.log
```
...
```bash
AWS_PROFILE=yn-dev aws cloudformation describe-stacks \
...
```bash
AWS_PROFILE=yn-dev aws rds describe-db-proxy-targets \
...
```bash
AWS_PROFILE=yn-dev aws wafv2 list-web-acls --scope REGIONAL --region ap-northeast-2 \
...
AWS_PROFILE=yn-dev aws cloudwatch describe-alarms \
  --alarm-name-prefix "Ym-Dev-Data-Proxy" \
  --query "MetricAlarms[].[AlarmName,StateValue]" --output table

# WAF BlockedRequests — Count mode 라 0 기대.
AWS_PROFILE=yn-dev aws cloudwatch get-metric-statistics \
  --namespace AWS/WAFV2 \
