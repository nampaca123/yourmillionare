...
1. **Foundation CMK in CDK**: 코드에서 KmsKeyId removal은 했지만 CFN drift 발생. Foundation을 다시 deploy해 secret resource definition을 정리하거나, CDK에서 `encryptionKey: alias/aws/secretsmanager`를 명시적으로 지정하는 게 정합성 측면에서 깔끔. (현재 운영 영향 없음)
...
3. **GCP OAuth Redirect URI**: Cognito Hosted UI 도메인 `https://yourmillionare-dev.auth.ap-northeast-2.amazoncognito.com/oauth2/idpresponse`를 GCP Console → OAuth Client → Authorized redirect URIs에 등록해야 실제 사용자가 Google로 로그인 가능. 본 E2E는 admin-initiate-auth로 이를 우회.
4. **CODEF 인증서/간편인증 마이그레이션**: 현재 loginType=1(ID/PW) — 평문 비밀번호가 Lambda 메모리 잠시 경유. Phase 1에서 인증서 팝업 또는 간편인증(Kakao/Toss/PASS) 채택해 사용자가 우리 서버에 자격증명을 넘기지 않도록 변경 예정.
