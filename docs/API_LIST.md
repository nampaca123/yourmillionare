...
| POST | `/tenants/{tenantId}/agent/search-tax-law` | HTTP API | JWT | 🟡 |
| POST | `/tenants/{tenantId}/agent/find-benefits` | HTTP API | JWT | 🟡 |
...
| 날짜 | 변경 |
|---|---|
| 2026-05-13 | **0023 unification**: journal_entry_draft DROP. 모든 분개가 단일 journal_entries 안에 confidence_status (certain/uncertain/discarded) 로 분류. `/journal/entries` GET → `/entries` 로 통합. `/uncertain*` 제거, `/entries/{id}/confirm` `/discard` PATCH 추가. 모든 reports/views 금액이 `{certain, uncertain, total}` AmountBreakdown 으로 분해 |
