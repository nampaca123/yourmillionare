...
data: {"type":"account","bankAccountId":"<uuid>","organization":"0088",
        "accountKind":"krw_demand","accountCurrency":"KRW",
        "accountNumberMasked":"********8154",
...
**FX (외화) 계좌 처리**: `accountKind="foreign"` 인 계좌도 같은 흐름으로 처리되지만 자동 분개를 만들지 않음:

- `account-list` 의 `resForeignCurrency` 로 link 한 계좌만 (CODEF) — `is_manual=true` 인 manual 외화계좌는 sync 대상이 아님
- `transaction-list` 호출 → 외화 금액(`resAccountIn` / `resAccountOut`)을 그대로 `fcy_amount` 로 저장. 통화 코드는 `tenant_bank_accounts.currency`
- `fx_observations` 의 closing rate (거래일 기준 on-or-before) 로 `fx_rate` 와 `amount`(KRW 환산) 계산. 적용 가능한 환율이 없으면 그 계정은 `outcome="codef_error"` + 환율 백필 안내 메시지
- `source="codef_fx"` (KRW 은 `codef_bank`)
- raw_transactions 는 즉시 `dispatched_at=now()` 로 마킹 → Bedrock classifier 로 enqueue 하지 않음. 따라서 SSE 응답에 FX 계좌발 `classification` 이벤트는 나오지 않음 (자동 분개 매핑은 후속 작업)
- `balance` SSE 이벤트의 `previous`/`current` 는 KRW 환산값 (snapshot 시점 환율)
...
### 5.6 외화 계좌 (Manual + CODEF FX Accounts)

> **목적**: 사용자가 직접 USD 외화 잔액을 등록(manual)하거나, CODEF에 이미 등록된 은행 연결(`tenant_bank_connections.connected_id`)을 재사용해 보유 중인 외화계좌를 발견·연결(CODEF)할 수 있다. FX 에이전트가 이 데이터를 보고 환전 전략을 제안한다. MVP 통화 화이트리스트: **USD 만**.
>
> **연결 흐름 (CODEF)**: KRW 계좌와 동일한 `account-list` 엔드포인트의 `resForeignCurrency` 배열을 사용하므로 별도 CODEF 인증 단계가 없다. 동일 은행 KRW 계좌를 먼저 `POST /tenants/{id}/bank-connections` 로 connect 한 뒤에야 그 은행의 외화계좌를 link 할 수 있다.
...
#### `GET /tenants/{tenantId}/fx/accounts/discoverable?organization=XXXX` 🟢

해당 (tenant, organization) 의 저장된 `connected_id` 를 재사용해 CODEF `account-list` 를 호출하고, 응답의 `resForeignCurrency[]` 를 정규화해서 반환한다. 사용자가 어떤 외화계좌가 있는지 미리 보고 link 대상을 고를 수 있도록 한다.

**Query**

| 이름 | 타입 | 비고 |
|---|---|---|
| `organization` | `string` (4-digit) | CODEF 기관코드 (예: `0088` 신한). 필수. |
  ...
  "accounts": [
    {
      ...
      "accountDisplay": "110-443-478154",
      "accountName": "외화보통예금",
      "currency": "USD",
      "balanceFcy": 1500.00,
      "supported": true,         // currency 가 USD MVP 화이트리스트에 해당하면 true
      "alreadyLinked": false      // 이미 tenant_bank_accounts 에 등록된 계좌면 true
    }
  ]
}
```
...
| 상황 | code | HTTP |
|---|---|---|
| `organization` 누락/형식 오류 | `VALIDATION_ERROR` | 422 |
| 해당 (tenant, organization) 의 `connected_id` 없음 — 먼저 `POST /bank-connections` 필요 | `NOT_FOUND` | 404 |
| CODEF API 호출 실패 | `CODEF_API_ERROR` | 502 |

#### `POST /tenants/{tenantId}/fx/accounts/link` 🟢

Discoverable 응답에서 사용자가 고른 외화계좌를 실제로 등록한다. `is_manual=false`, `account_kind='foreign'`, `connected_id` 는 `tenant_bank_connections` 에서 복사. 호출 시 CODEF 를 재호출해 잔액·통화·존재 여부를 서버에서 한 번 더 검증한 뒤 insert.

**Request body**

```json
{
  "organization": "0088",          // required, 4-digit CODEF org
  "accountNumber": "110443478154", // required, discover 응답의 accountNumber
  "bankLabel": "Shinhan USD"        // optional, 1~40 chars
}
```
...
```json
{
  ...
  "balanceFcy": null,                // CODEF 연동 계좌는 FCY 별도 저장하지 않음 (link 시점 KRW 환산만 기록)
  "balanceKrwToday": 2226450,        // = balanceFcy(link 시) × 최신 fx_observations USD/KRW
  "lastSyncedAt": "2026-05-13T..."
}
```
...
| 상황 | code | HTTP |
|---|---|---|
| `organization` / `accountNumber` 누락·형식 오류 | `VALIDATION_ERROR` | 422 |
| 해당 (tenant, organization) 의 `connected_id` 없음 | `NOT_FOUND` | 404 |
| CODEF 응답에 해당 `accountNumber` 의 외화계좌 없음 | `NOT_FOUND` | 404 |
| 통화가 USD 가 아님 (MVP 화이트리스트 위반) | `VALIDATION_ERROR` | 422 |
| 이미 (tenant, organization, accountNumber) 가 등록됨 | `CONFLICT` | 409 |
| CODEF API 호출 실패 | `CODEF_API_ERROR` | 502 |
...
외화계좌 soft delete (`is_active=false`). manual / CODEF link 양쪽 모두 지원. idempotent — 이미 비활성이면 그대로 204. CODEF link 의 경우 `tenant_bank_connections.connected_id` 자체는 삭제하지 않으며 (다른 계좌 연결에 재사용), 이후 sync 에서 이 계좌는 제외된다.
...
| 상황 | code | HTTP |
|---|---|---|
| 처음부터 없는 ID 또는 `account_kind != 'foreign'` | `NOT_FOUND` | 404 |
...
| GET | `/tenants/{tenantId}/fx/accounts/discoverable` | HTTP API | JWT | 🟢 |
| POST | `/tenants/{tenantId}/fx/accounts/link` | HTTP API | JWT | 🟢 |
