// Function URL Lambda: SSE-streams a Bedrock tool_use agent for fixed FX-strategy scenarios. No free-text input.
...
import { buildGetExtendedRateHistoryTool } from '../../../application/tools/get-extended-rate-history.tool.js';
import {
  FX_SCENARIOS,
  buildFxContext,
  buildFxUserMessage,
  getFxSystemPrompt,
  isFxScenario,
} from '../../../application/fx-strategy-templates.js';

const RequestBodySchema = z
  .object({
    tenantId: z.string().uuid(),
    scenario: z.enum(FX_SCENARIOS),
  })
  .strict();
...
const SCENARIO_PATH_PARAM_RE = /^\/tenants\/([0-9a-f-]+)\/fx\/strategy\/?$/i;
...
const handlerImpl = async (event: FunctionUrlEvent, responseStream: ResponseStream): Promise<void> => {
  ...
  if (!isFxScenario(parsed.data.scenario)) {
  ...
  const agentContext = await buildFxContext({
    pool: getPool(),
    tenantId,
  ...
  const tools: Tool[] = [buildGetExtendedRateHistoryTool(getPool()) as Tool];
  ...
  try {
    result = await runAgent({
      systemPrompt: getFxSystemPrompt(),
      userMessage: buildFxUserMessage(parsed.data.scenario, agentContext),
      tools,
      ctx,
...
const guardedHandler = withStreamingErrorBoundary({ path: '/fx/strategy' }, handlerImpl);
