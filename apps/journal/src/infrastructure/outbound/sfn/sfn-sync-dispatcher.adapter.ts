...
export class SfnSyncDispatcher implements SyncDispatcher {
  ...
  constructor(arn?: string, region?: string) {
    ...
    this.client = new SFNClient({ region: region ?? process.env.APP_REGION ?? process.env.AWS_REGION ?? 'ap-northeast-2' });
  }

  async start(input: { tenantId: string; idempotencyKey?: string }): Promise<{ executionArn: string; startDate: string }> {
    if (!this.stateMachineArn) {
      throw new ValidationError('MANUAL_SYNC_STATE_MACHINE_ARN env is not configured');
    }
    const command = new StartExecutionCommand({
      stateMachineArn: this.stateMachineArn,
      input: JSON.stringify({ tenantId: input.tenantId, triggeredBy: 'manual' }),
      ...(input.idempotencyKey ? { name: `manual-${input.tenantId}-${input.idempotencyKey.slice(0, 30)}` } : {}),
    });
