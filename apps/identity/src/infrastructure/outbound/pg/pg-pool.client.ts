// IAM-authenticated pg.Pool with token caching and concurrent-refresh deduplication.
...
const doRefresh = async (): Promise<Pool> => {
  ...
  pool = new Pool({
    ...
    // max:1 per Lambda instance — RLS context is set per-connection, sharing is unsafe.
    max: 1,
  });
