// Client: fetches and caches CODEF OAuth access token for the Identity Lambda bundle.

import { GetSecretValueCommand, SecretsManagerClient } from '@aws-sdk/client-secrets-manager';
import { AppError } from '@ym/shared-errors';

interface CodefSecretValue {
  clientId: string;
  clientSecret: string;
  publicKey: string;
}
...
let cachedSecret: CodefSecretValue | undefined;
let cachedToken: string | undefined;
let tokenExpiresAt = 0;

const fetchSecret = async (): Promise<CodefSecretValue> => {
  ...
  cachedSecret = JSON.parse(result.SecretString) as CodefSecretValue;
  return cachedSecret;
};

const fetchToken = async (secret: CodefSecretValue): Promise<CodefTokenResponse> => {
...
export const getCodefIdentityToken = async (): Promise<string> => {
...
export const getCodefIdentitySecret = async (): Promise<CodefSecretValue> => fetchSecret();
